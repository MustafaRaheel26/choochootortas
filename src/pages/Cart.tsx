  ...
  if (cart.length === 0) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center p-12 text-center bg-black">
        ...
        <h2 className="text-[48px] font-black uppercase tracking-tighter text-white mb-6">{t('empty_bag')}</h2>
        <p className="text-xl text-muted font-bold uppercase tracking-[0.2em] max-w-md mb-12 leading-relaxed">
          {t('add_something')}
        </p>
        <Button variant="primary" size="xl" className="px-24 h-24 rounded-full text-2xl" onClick={onBack}>
          {t('browse_menu')}
        </Button>
      </div>
    );
          ...
          <div className="flex justify-between items-end border-b border-primary/20 pb-8 mb-10">
            <div>
              <div className="flex items-center gap-4 mb-2">
                <h2 className="text-[48px] font-black uppercase tracking-tighter text-white leading-none">{t('my_order')}</h2>
                <div className={`px-4 py-1 rounded-full border ${orderType === 'take-out' ? 'border-accent text-accent' : 'border-primary text-primary'} text-[10px] font-black uppercase tracking-widest`}>
                  {orderType === 'take-out' ? 'TAKE OUT' : 'EAT IN'}
                </div>
              </div>
              <p className="text-xl font-bold text-white/40 uppercase tracking-[0.4em]">{cart.length} {t('items_selected')}</p>
            </div>
          </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="text-right">
                    <p className="text-[8px] font-black text-primary uppercase tracking-[0.3em] mb-1">Item Total</p>
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2 mt-4">
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">{t('qty')}</span>
                    <span className="text-lg font-black text-white">x{item.quantity}</span>
                  </div>

                  {item.removedIngredients.length > 0 && (
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[9px] font-black text-red-500 uppercase tracking-widest">{t('removed')}</span>
                  ...
                  {item.addedExtras.length > 0 && (
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[9px] font-black text-primary uppercase tracking-widest">{t('extras')}</span>
                ...
                {item.notes && (
                  <div className="mt-3 pt-2 border-t border-white/5">
                    <p className="text-[10px] font-bold text-muted italic leading-tight truncate">
                      <span className="text-[8px] font-black text-white/20 uppercase tracking-widest not-italic mr-2">{t('notes')}:</span>
                      "{item.notes}"
                    </p>
                  </div>
                )}
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="pt-4 border-t border-white/5 flex justify-between items-center">
              <span className="text-primary font-black uppercase tracking-[0.4em] text-sm">{t('grand_total')}</span>
